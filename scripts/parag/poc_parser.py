...
from search_stack.parag.parser import parse

DEFAULT_DB = Path.home() / ".swiss-caselaw" / "decisions.db"


def sample_bge(db_path: Path, n: int, min_len: int = 8000, seed: int | None = None) -> list[tuple[str, str, str]]:
    conn = sqlite3.connect(str(db_path))
    try:
        if seed is not None:
            # deterministic sampling: order by hash(decision_id, seed)
            rows = conn.execute(
                """
                SELECT decision_id, language, full_text
                FROM decisions
                WHERE court='bge' AND full_text IS NOT NULL AND length(full_text) > ?
...
def format_report(parsed_list: list) -> str:
    ...
    for p in parsed_list:
        ...
        lines.append(
            f"\n─── {p.decision_id}  lang={p.language}  len={p.text_length} ───"
        )
    ...
    lines.append(
        f"  {overall['n_considerants_total']} total considérants across all decisions"
    )
    return "\n".join(lines)
...
def main():
    ...
    samples = sample_bge(args.db, args.n, args.min_len)
    if not samples:
        print("No samples found. Check DB path and court='bge' availability.")
        return

    parsed = [parse(did, lang, txt) for did, lang, txt in samples]
